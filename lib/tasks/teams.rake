...
# add players
  ...
  ncaa_player_array.each do |team|
    team_index +=1
    team.each do |player|
      Player.update(player_id, name: player[0].encode( "UTF-8", "binary", :invalid => :replace, :undef => :replace), gp: player[1] , min: player[2] , ppg: player[3] , rpg: player[4] , apg: player[5] , spg: player[6] , bpg: player[7] , tpg: player[8] , fg: player[9] , ft: player[10] , threep: player[11], team_id: index )
      player_id+=1
    end
  end
